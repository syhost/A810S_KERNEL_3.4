...
/* ICS upgrade 수정 사항은 아래 피처를 사용한다 */
#define F_PANTECH_CAMERA_FIX_FOR_ICS
...
//#define F_PANTECH_CAMERA_CFG_COLOREXTRACTION //성능 이슈에 따라 색추출 기능 삭제 2012.06.05
#endif

/* 
 * Qualcomm SR for testVideoSnapshot of CTS
 */ 
#define F_PANTECH_CAMERA_LIVESNAPSHOT_PICTURESIZE
...
/* camera app에서 camera 종료시 QCP 종료가 끝나기전 다시 camera start를 시도할 경우 발생하는 singleton문제로 camra preview진입 fail발생할 경우,   
    단말 재 부팅전까지 camera preview진입 안되는 현상에 대한 보안 code.
    PRESTO에서 runFrameThread()의 실제 생성이 늦어 camera종료 루틴 보다 실제 생성이 늦게 될경우 ~QualcommCameraHardware()소멸자가 호출 안되어 singleton문제 발생함.
*/
#define F_PANTECH_CAMERA_FRAMETHREAD_SINGLETON

/*캡쳐 command가 내려와 캡쳐 진행 중 다시 또 캡쳐 command가 내려오는 경우 
HAL 단에서 lock이 걸려 ANR 발생 및 재진입 실패에 관한 방어코드 적용*/
#define F_PANTECH_CAMERA_FIX_SNAPSHOT_INFINITY_LOCK
