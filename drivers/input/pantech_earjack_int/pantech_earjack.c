...
//#if defined(CONFIG_SKY_EF40S_BOARD) // N1037 20120730 for new touch driver 
...
//	{ "KEY_MEDIA",		KEY_MEDIA,		60,	130},
	{ "KEY_MEDIA",		KEY_MEDIA,		50,	130}, //20120412_kmh_sensor
...
//	{ "KEY_MEDIA_CARKIT",	KEY_MEDIA,		0,	50}, // car_kit - should be on the last position 
	{ "KEY_MEDIA_CARKIT",	KEY_MEDIA,		0,	40}, // car_kit - should be on the last position  //20120412_kmh_sensor
};
#endif
...
int irq_state;      //20120705_N1019_ef40s protectcode
...
//20120705_N1019_ef40s protectcode
...
static int __devinit pantech_earjack_probe(struct platform_device *pdev)
{
	...
	irq_state = 0;    //20120705_N1019_ef40s protectcode
...
// change from "h2w because same name pmic8058-othc.c PS2 P13106
       earjack->sdev.name	= "hw2";      // N1037 20120402 for ICS earjack 
	...
	disable_irq_detect(); //disable_irq_nosync(gpio_to_irq(EARJACK_DET));  20120705_N1019_ef40s protectcode
...
static void earjack_detect_func( struct work_struct *test_earjack)         
{
	...
	enable_irq_detect(); //enable_irq(gpio_to_irq(EARJACK_DET));   20120705_N1019_ef40s protectcode
